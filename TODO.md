...
- [x] ~~Root README.md describes the Cloud Run era~~ — DONE 2026-08-08:
      README rewritten for the Actions+raw architecture, and the entire
      legacy stack (app.py, build_local.py, Dockerfile, deploy.sh,
      serve-local.sh, templates/, site/, storage.py's GCS backend, Flask/
      gunicorn/google-cloud-storage deps) was deleted — git history has it.
- [x] ~~ios/README.md "Cloud Run backend" intro line~~ — already fixed; no
      Cloud Run mention remains in ios/README.md.
