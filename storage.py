...
Cloud Storage. Lets the Cloud Host service serve immediately on cold start and
survive scale-to-zero without re-scraping, and preserves the previous data if a
scrape fails.

If BUCKET is unset (e.g. local dev without object storage), these become no-ops returning
...
def _blob(name: str):
    from objectstore import storage  # imported lazily so local dev needs no creds
...
def load(name: str) -> dict | None:
    ...
    try:
        ...
        log.warning("object storage load failed for %s: %r", name, e)
        return None
...
def save(name: str, payload: dict) -> bool:
    ...
    try:
        ...
        log.warning("object storage save failed for %s: %r", name, e)
        return False
