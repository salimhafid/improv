...
Routes:
  ...
  GET  /shows.json  canonical JSON payload — reads the object storage store fresh (no scrape)
  GET  /status      health check (path /healthz is swallowed by an org proxy)
  POST /refresh     re-scrape sources, update object storage + in-memory cache
...
object storage store. Reads (GET /shows.json, the app's pull-to-refresh, the web page) serve
the latest from the object storage store so updates show up without triggering a scrape.
"""
...
# Serve the in-memory copy for speed; refresh it from the object storage store at most every
...
def ensure_loaded() -> dict:
    """Return the cached payload, loading from object storage or scraping if needed."""
...
def latest_payload() -> dict:
    ...
    re-reads the object storage store when the copy is older than _FRESH_TTL, so reads are
    fast while still reflecting the hourly scheduled scrape. Falls back to the
    in-memory copy if object storage is briefly unavailable, and only bootstraps a one-time
    scrape on a truly cold start (no object storage object yet)."""
    if _cache["payload"] is not None and (time.monotonic() - _last_read["at"]) < _FRESH_TTL:
        return _cache["payload"]
    payload = storage.load_payload()   # object storage read; None on miss/failure, never scrapes
...
def latest_classes() -> dict:
    """Freshest classes payload from the object storage store (no scrape), in-memory TTL."""
